# langchaintestpdf
import streamlit as st

# File Processing
from PIL import Image
import pandas as pd
import docx2txt
import PyPDF2 
# from PyPDF2 import PdfReader
# from PyPDF2 import PdfFileReader
import pdfplumber




# Load Image
def load_image(image_file):
    img = Image.open(image_file)
    return img

# PDF Reader
def read_pdf(file):
    pdfReader = PyPDF2.PdfReader(file)
    num_pages = len(pdfReader.pages)
    print(f'Total Pages:{num_pages}')
    # count = pdfReader.numPages
    # all_page_text = ""
    for num_page in range(len(pdfReader.pages)):
        page = pdfReader.pages[num_page]
        text = page.extract_text()
    return text


def main():
    st.title("File Upload")

    menu = ["About","Image","Dataset","DocumentFiles"]
    choice =st.sidebar.selectbox("Menu",menu)

    # # PDF Reader
    # if choice == "Pdf Document":
    #     st.subheader("Pdf Document")
    #     pdf_file = st.file_uploader("Upload your PDF",type='pdf')
    #     if pdf_file is not None:
    #         # st.write(type(pdf_file))
    #         # Method/Attributes
    #         # st.write(dir(pdf_file))
    #         file_details = {"File Name": pdf_file.name,"File Type":pdf_file.type,"File Size": pdf_file.size}
    #         st.write(file_details)

    # Image Reader
    if choice == "Image":
        st.subheader("Image")
        image_file = st.file_uploader("Upload Image",
                    type=["png","jpeg","jpg"])
        # st.write(type(image_file))
        # Image Methods/Attributes
        # st.write(dir(image_file))    
        file_details = {"File Name": image_file.name,"File Type": image_file.type,
                        "File Size":image_file.size}
        st.write(file_details)
        st.image(load_image(image_file))

    # Data Reader csv, xls
    elif choice == "Dataset":
        st.subheader("Dataset")
        data_file = st.file_uploader("Upload CSV", type=["csv","xls"])
        if data_file is not None:
            # st.write(type(data_file))
            file_details = {"Data Name": data_file.name,"Data Type": data_file.type,
                        "Data Size":data_file.size}
            st.write(file_details)
            df =pd.read_csv(data_file)
            st.dataframe(df)

    # Docs File 
    elif choice == "DocumentFiles":
        st.subheader("DocumentFile")
        docx_file = st.file_uploader("Upload Document",
                    type=["pdf","docx","txt"])
        
        if st.button("Process"):
            if docx_file is not None:
                file_details = {"File Name": docx_file.name,"File Type": docx_file.type,
                        "File Size":docx_file.size}
                st.write(file_details)
                if docx_file.type == "text/plain":
                    # Read as bytes
                    # raw_text = docx_file.read()
                    # st.write(raw_text) #works but in bytes
                    # st.text(raw_text)
                    
                    # Read as String(decode bytes to string)
                    raw_text =str(docx_file.read(),"utf-8")
                    st.write(raw_text) # works fine
                    # st.text(raw_text)
                
                # PDF Reader
                elif docx_file.type == "application/pdf":
                    # try:
                    #     with pdfplumber.open(docx_file) as pdf:
                    #         pages = pdf.pages[0]
                    #         st.write(pages.extract_text())
                    # except:
                    #     st.write("None")

                    # Using PyPDF
                    raw_text = read_pdf(docx_file)
                    st.write(raw_text)


                # Docx Reader
                else:
                    raw_text = docx2txt.process(docx_file)
                    st.write(raw_text)
                    # st.text(raw_text)


    else:
        st.subheader("About")
    

if __name__ == '__main__':
    main()
